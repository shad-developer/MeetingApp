...
const DataTable = ({
    ...
    return (
        <>
            <div className="p-4 mb-4 bg-gray-50 border-amber-500 border-2 shadow-sm flex justify-between items-center rounded-2xl">
                ...
                <div className="flex items-center gap-3">
                    ...
                    {Object.keys(selectedItems).length > 0 && (
                        <button onClick={() => onDeleteSelected(Object.keys(selectedItems))} className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-white bg-amber-500 hover:bg-amber-600  transition-colors rounded-md">
            ...
            <div className="flex-1 flex gap-4 overflow-hidden">
                <div className="flex-grow flex flex-col rounded-2xl shadow-md bg-gray-50 overflow-hidden min-w-0 transition-all duration-300 h-screen border-amber-500 border-2">
                                ...
                                <thead className="sticky top-0 bg-gray-50 z-10">
                                    <tr className="border-b border-gray-200 text-gray-600">
                                        ...
                                        {columns.filter(c => c.visible).map(column => (
                                            <th key={column.field} style={{ width: `${column.width || 150}px` }} className="relative px-4 py-2 text-left text-md font-bold group">
                        ...
                        <div className="px-6 space-y-1 max-h-[60vh] overflow-y-auto">
                            {columns.map(c => (
                                <div key={c.field} className="flex items-center gap-3 p-2.5 rounded-md hover:bg-gray-100 cursor-pointer transition-colors ">
                                    ...
                                    />
                                    <span className="text-md text-gray-700 select-none font-semibold">{c.label}</span>
                                </div>
                            ))}
                        </div>
